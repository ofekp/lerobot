...
def train(cfg: TrainPipelineConfig, accelerator: Accelerator | None = None):
    ...
    # Deterministic mode: sacrifice some performance for reproducibility.
    # When a seed is set, force fully deterministic CUDA operations so that
    # two training runs with the same seed produce bit-identical results.
    if cfg.seed is not None:
        import os
    ...
    for _ in range(step, cfg.steps):
        ...
        if is_log_step:
            ...
            _voxel_enc = getattr(getattr(_groot_model, "backbone", None), "voxel_encoder", None) if _groot_model else None
            if _voxel_enc is not None and hasattr(_voxel_enc, "_last_voxel_grid"):
                try:
                    from lerobot.policies.groot.voxel_encoder import render_voxel_projections
                    from PIL import Image
                    viz_dir = Path(cfg.output_dir) / "voxel_viz"
                    viz_dir.mkdir(parents=True, exist_ok=True)
                    img_arr = render_voxel_projections(_voxel_enc._last_voxel_grid)
